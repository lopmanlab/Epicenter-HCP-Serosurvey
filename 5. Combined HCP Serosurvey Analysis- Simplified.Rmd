...
# merge combined (and university-specific results)
  uni_results_combined <- rbind(uni_results)
...
# test results by week
  ...
  # reorder columns
    uni_results_week <- uni_results_week[, c(2,3,4)]
  # print table  
...
# bar plot for week of test by university and line with percent positive
  ...
  # plot
    ...
    pdf("/Users/juliabaker/Box Sync/Combined HCP serosurvey/Manuscripts/JAMA Network Open/Revision/Figure_1.pdf", width=7, height=5)
...
# mean and range for cumulative incidence
  ...
  #summary(all_pos.neg$cum_inc)
  #summary(only_emory$cum_inc)
  #summary(only_maryland$cum_inc)
  #summary(only_hopkins$cum_inc)
  #summary(only_rush$cum_inc)
...
# model 1- Oct 26 (updated Nov to change "occ_cont" to "occ_cont_mod")
  m1 <- glm(orig.result ~ sex + age + race + eth + 
                          log_cum_inc + com_cont + 
                          job_role + envir_mod2 + occ_cont_mod,
                          data = all_pos.neg, family = "binomial")

  # results  
    #summary(m1)
    #1277 observations deleted due to missingness
    m1_coeff <- exp(cbind(OR = coef(m1), confint(m1)))
    m1_p_val <- summary(m1)$coefficients[,4]
    m1_results <- data.frame(round((cbind(m1_coeff, m1_p_val)), 3))
      
  # formatting
    # rename/reorder columns
      setDT(m1_results, keep.rownames = "category")
      names(m1_results)[names(m1_results) == "X2.5.."] <- "LCI"
      names(m1_results)[names(m1_results) == "X97.5.."] <- "UCI"
      names(m1_results)[names(m1_results) == "m1_p_val"] <- "p_val"
   # combining OR and CIs into one column
      m1_results$OR_CI_part1 <- paste(format(round(m1_results$OR, digits=1), nsmall=1), 
                                         format(round(m1_results$LCI, digits=1), nsmall=1), sep=" (")
      m1_results$OR_CI_part2 <- paste(m1_results$OR_CI_part1, 
                                         format(round(m1_results$UCI, digits=1), nsmall=1), sep="-")
      m1_results$OR_CI_part3 <- ")"
      m1_results$adj_OR_CI <- paste(m1_results$OR_CI_part2, m1_results$OR_CI_part3, sep="")
        m1_results[ ,c('OR_CI_part1', 'OR_CI_part2', 'OR_CI_part3', 'OR', 'LCI', 'UCI')] <- list(NULL)
        m1_results <- m1_results[, c(1,3)]  
...
# model 1- all except Emory
  m1_except_eu <- glm(orig.result ~ sex + age + race + eth + 
                                    log_cum_inc + com_cont + 
                                    job_role + envir_mod2 + occ_cont_mod,
                                    data = all_except_emory, family = "binomial")

  # results  
    #summary(m1_except_eu)
    #689 observations deleted due to missingness
    m1_except_eu_coeff <- exp(cbind(OR = coef(m1_except_eu), confint(m1_except_eu)))
    m1_except_eu_p_val <- summary(m1_except_eu)$coefficients[,4]
    m1_except_eu_results <- data.frame(round((cbind(m1_except_eu_coeff, m1_except_eu_p_val)), 3))
      
  # formatting
    # rename/reorder columns
      setDT(m1_except_eu_results, keep.rownames = "category")
      names(m1_except_eu_results)[names(m1_except_eu_results) == "X2.5.."] <- "LCI"
      names(m1_except_eu_results)[names(m1_except_eu_results) == "X97.5.."] <- "UCI"
      names(m1_except_eu_results)[names(m1_except_eu_results) == "m1_except_eu_p_val"] <- "p_val"
    # fix very high CI
      m1_except_eu_results$UCI <- ifelse(m1_except_eu_results$category == "sexOther/Unknown", NA, m1_except_eu_results$UCI)
      m1_except_eu_results$UCI <- ifelse(m1_except_eu_results$category == "com_contUnknown", NA, m1_except_eu_results$UCI)
    # combining OR and CIs into one column
      m1_except_eu_results$OR_CI_part1 <- paste(format(round(m1_except_eu_results$OR, digits=1), nsmall=1, trim=TRUE), 
                                         format(round(m1_except_eu_results$LCI, digits=1), nsmall=1, trim=TRUE), sep=" (")
      m1_except_eu_results$OR_CI_part2 <- paste(m1_except_eu_results$OR_CI_part1, 
                                         format(round(m1_except_eu_results$UCI, digits=1), nsmall=1, trim=TRUE), sep="-")
      m1_except_eu_results$OR_CI_part3 <- ")"
      m1_except_eu_results$aOR_no_EU <- paste(m1_except_eu_results$OR_CI_part2, m1_except_eu_results$OR_CI_part3, sep="")
          m1_except_eu_results[ ,c('OR_CI_part1', 'OR_CI_part2', 'OR_CI_part3', 'OR', 'LCI', 'UCI')] <- list(NULL)
          m1_except_eu_results <- m1_except_eu_results[, c(1,3)]     
...
# random intercept
  ...
  all_pos.neg_no_miss <- all_pos.neg %>% filter(sex != "Other/Unknown" &
                                                race != "Unknown" &
                                                eth != "Unknown" &
                                                job_role != "Unknown" &
                                                occ_cont != "Unknown" &
  ...
  # formatting
   ...
   # combining OR and CIs into one column
      ...
      m4_results$aOR_m4 <- paste(m4_results$OR_CI_part2, m4_results$OR_CI_part3, sep="")
          m4_results[ ,c('OR_CI_part1', 'OR_CI_part2', 'OR_CI_part3', 'OR', 'LCI', 'UCI')] <- list(NULL)
  ...
  # formatting
   ...
   # combining OR and CIs into one column
      ...
      m4_except_eu_results$aOR_m4_except_eu <- paste(m4_except_eu_results$OR_CI_part2, m4_except_eu_results$OR_CI_part3, sep="")
          m4_except_eu_results[ ,c('OR_CI_part1', 'OR_CI_part2', 'OR_CI_part3', 'OR', 'LCI', 'UCI')] <- list(NULL)
...
# creating linear time variable to account for possible time trend
  all_pos.neg$month <- ifelse((all_pos.neg$week == 17 | all_pos.neg$week == 18), 4, 
                       ifelse((all_pos.neg$week == 19 | all_pos.neg$week == 20 | all_pos.neg$week == 21 | all_pos.neg$week == 22 | all_pos.neg$week == 23), 5,
                       ifelse((all_pos.neg$week == 24 | all_pos.neg$week == 25 | all_pos.neg$week == 26 | all_pos.neg$week == 27), 6,
                       ifelse((all_pos.neg$week == 28 | all_pos.neg$week == 29 | all_pos.neg$week == 30 | all_pos.neg$week == 31), 7,
                       ifelse((all_pos.neg$week == 32 | all_pos.neg$week == 33 | all_pos.neg$week == 34 | all_pos.neg$week == 35 | all_pos.neg$week == 36), 8, NA)))))
  
  all_pos.neg$month_cat <- ifelse((all_pos.neg$week == 17 | all_pos.neg$week == 18), "Apr", 
                           ifelse((all_pos.neg$week == 19 | all_pos.neg$week == 20 | all_pos.neg$week == 21 | all_pos.neg$week == 22 | all_pos.neg$week == 23), "May",
                           ifelse((all_pos.neg$week == 24 | all_pos.neg$week == 25 | all_pos.neg$week == 26 | all_pos.neg$week == 27), "Jun",
                           ifelse((all_pos.neg$week == 28 | all_pos.neg$week == 29 | all_pos.neg$week == 30 | all_pos.neg$week == 31), "Jul",
                           ifelse((all_pos.neg$week == 32 | all_pos.neg$week == 33 | all_pos.neg$week == 34 | all_pos.neg$week == 35 | all_pos.neg$week == 36), "Aug", NA)))))
  all_pos.neg$month_cat <- relevel(as.factor(all_pos.neg$month_cat), ref = "Apr") 
  
  
  
# model 4 (random intercept model) with linear variable for time (month)
  m4_time <- glmer(orig.result ~ sex + age + race + eth + 
                                 log_cum_inc + com_cont + 
                                 job_role + envir_mod2 + occ_cont_mod +
                                 month_cat +
                                 (1 | univ), data = all_pos.neg, family = binomial, control = glmerControl(optimizer = "bobyqa"))
  ...
  # results  
    summary(m4_time)
    m4_time_se <- sqrt(diag(vcov(m4_time)))
    m4_time_results <- data.frame(exp(cbind(OR = fixef(m4_time), LCI = fixef(m4_time) - 1.96*m4_time_se, UCI = fixef(m4_time) + 1.96*m4_time_se)))
  # formatting
    # rename/reorder columns
      setDT(m4_time_results, keep.rownames = "category")
    # fix very high CI
      m4_time_results$UCI <- ifelse(m4_time_results$category == "sexOther/Unknown", NA, m4_time_results$UCI)
   # combining OR and CIs into one column
      m4_time_results$OR_CI_part1 <- paste(format(round(m4_time_results$OR, digits=1), nsmall=1, trim=TRUE), 
                                         format(round(m4_time_results$LCI, digits=1), nsmall=1, trim=TRUE), sep=" (")
      m4_time_results$OR_CI_part2 <- paste(m4_time_results$OR_CI_part1, 
                                         format(round(m4_time_results$UCI, digits=1), nsmall=1, trim=TRUE), sep="-")
      m4_time_results$OR_CI_part3 <- ")"
      m4_time_results$aOR_m4_time <- paste(m4_time_results$OR_CI_part2, m4_time_results$OR_CI_part3, sep="")
          m4_time_results[ ,c('OR_CI_part1', 'OR_CI_part2', 'OR_CI_part3', 'OR', 'LCI', 'UCI')] <- list(NULL)
...
# merge each university and combined
  # format column name
    m1_all <- m1_results
    names(m1_all)[names(m1_all) == "adj_OR_CI"] <- "aOR_all"
          
  all_results <- merge(m1_results, m4_results, by="category", all=TRUE)
  all_results <- merge(all_results, m4_time_results, by="category", all=TRUE)

  
    # create "variable" column
    all_results$variable <-  ifelse((all_results$category %like% "week"), "week",
                                  ifelse((all_results$category %like% "race"), "race",
                                  ifelse((all_results$category %like% "eth"), "eth",
                                  ifelse((all_results$category %like% "log_cum_inc"), "log_cum_inc",
                                  ifelse((all_results$category %like% "age"), "age",
                                  ifelse((all_results$category %like% "univ"), "univ",
                                  ifelse((all_results$category %like% "fac_type"), "fac_type",
                                  ifelse((all_results$category %like% "fac_size"), "fac_size",
                                  ifelse((all_results$category %like% "job_role"), "job_role",
                                  ifelse((all_results$category %like% "envir_mod2"), "envir_mod2",
                                  ifelse((all_results$category %like% "envir_mod"), "envir_mod",
                                  ifelse((all_results$category %like% "envir"), "envir",       
                                  ifelse((all_results$category %like% "loc_covid"), "loc_covid",
                                  ifelse((all_results$category %like% "loc_ed"), "loc_ed",
                                  ifelse((all_results$category %like% "loc_test"), "loc_test",
                                  ifelse((all_results$category %like% "occ_cont_mod"), "occ_cont_mod",
                                  ifelse((all_results$category %like% "agp"), "agp",
                                  ifelse((all_results$category %like% "com_cont"), "com_cont", 
                                  ifelse((all_results$category %like% "sex"), "sex", 
                                  ifelse((all_results$category %like% "symp"), "symp",
                                  ifelse((all_results$category %like% "prior_test"), "prior_test", 
                                  ifelse((all_results$category %like% "month"), "month", NA))))))))))))))))))))))

    # format "category" column
      all_results$category <- str_remove(all_results$category, "race") 
      all_results$category <- str_remove(all_results$category, "sex") 
      all_results$category <- str_remove(all_results$category, "eth") 
      all_results$category <- str_remove(all_results$category, "age") 
      all_results$category <- str_remove(all_results$category, "univ") 
      all_results$category <- str_remove(all_results$category, "fac_type")
      all_results$category <- str_remove(all_results$category, "fac_size") 
      all_results$category <- str_remove(all_results$category, "job_role") 
      all_results$category <- str_remove(all_results$category, "envir_mod2")
      all_results$category <- str_remove(all_results$category, "envir_mod")
      all_results$category <- str_remove(all_results$category, "envir")       
      all_results$category <- str_remove(all_results$category, "loc_covid")       
      all_results$category <- str_remove(all_results$category, "loc_ed")       
      all_results$category <- str_remove(all_results$category, "loc_test") 
      all_results$category <- str_remove(all_results$category, "occ_cont_mod")
      all_results$category <- str_remove(all_results$category, "agp")
      all_results$category <- str_remove(all_results$category, "com_cont")
      all_results$category <- str_remove(all_results$category, "symp")
      all_results$category <- str_remove(all_results$category, "prior_test")
      all_results$category <- str_remove(all_results$category, "month_cat")
      
      
  # format       
    all_results$variable <- ifelse((all_results$variable %like% "week"), "Week of test",
                                     ifelse((all_results$variable %like% "race"), "Race",
                                     ifelse((all_results$variable %like% "eth"), "Ethnicity",
                                     ifelse((all_results$variable %like% "log_cum_inc"), "Cumulative incidence (log10)",
                                     ifelse((all_results$variable %like% "age"), "Age group",
                                     ifelse((all_results$variable %like% "univ"), "University",
                                     ifelse((all_results$variable %like% "fac_type"), "Facility type",
                                     ifelse((all_results$variable %like% "fac_size"), "Facility size",
                                     ifelse((all_results$variable %like% "job_role"), "Job role",
                                     ifelse((all_results$variable %like% "envir_mod2"), "Work environment (modified v2)",
                                     ifelse((all_results$variable %like% "envir_mod"), "Work environment (modified)",
                                     ifelse((all_results$variable %like% "envir"), "Work environment",       
                                     ifelse((all_results$variable %like% "loc_covid"), "Worked in COVID-19 care location",
                                     ifelse((all_results$variable %like% "loc_ed"), "Worked in ED",
                                     ifelse((all_results$variable %like% "loc_test"), "Worked in COVID-19 testing area",
                                     ifelse((all_results$variable %like% "occ_cont_mod"), "COVID-19 patient contact (modified)",
                                     ifelse((all_results$variable %like% "agp"), "Aerosol generating procedures",
                                     ifelse((all_results$variable %like% "com_cont"), "Community contact", 
                                     ifelse((all_results$variable %like% "sex"), "Sex", 
                                     ifelse((all_results$variable %like% "symp"), "Symptoms",
                                     ifelse((all_results$variable %like% "prior_test"), "Prior test",
                                     ifelse((all_results$variable %like% "result"), "Result", 
                                     ifelse((all_results$variable %like% "month"), "Month of test", NA)))))))))))))))))))))))    
  ...
  # ordering the variables/levels
    variable_order <- c("Sex", "Age group", "Ethnicity", "Race", 
                        ...
                        "Aerosol generating procedures", "COVID-19 patient contact (modified)",
                        "Cumulative incidence (log10)", "Community contact", "Symptoms", "Prior test", "Week of test", "Month of test", "Result")
      
 # apply orders to dataframe     
   all_results$variable <- ordered(all_results$variable, levels=variable_order)
   all_results <- all_results[with(all_results, order(variable, category)),]
 # reorder columns
   all_results <- all_results[, c(5,1,2,3,4)]     
   
  # export full table
    write.csv(all_results,"/Users/juliabaker/Box Sync/Combined HCP serosurvey/Analysis/all_results.csv", row.names = FALSE)
